...
contract SanwoRouterV1 {
    ...
    modifier onlyOwner() {
        require(
            configuration.owner() == msg.sender,
    ...
    function route(
        ...
        uint256 balanceBefore = _balanceBefore(address(desc.dstToken));
    ...
    function _balanceBefore(address token) private returns (uint256 balance) {
    ...
    function _ensureTransferIn(address tokenIn, uint256 amountIn) private {
        if (tokenIn == ETH) {
            require(
                msg.value >= amountIn,
    ...
    function _execute(
        ...
        for (uint256 i = 0; i < plugins.length; i++) {
            ...
            ISanwoRouterV1Plugin plugin = ISanwoRouterV1Plugin(
                configuration.approvedPlugins(plugins[i])
            );

            if (plugin.delegate()) {
                (bool success, bytes memory returnData) = address(plugin)
                    .delegatecall(
                        abi.encodeWithSelector(
                            plugin.execute.selector,
                            caller,
                            desc,
                            data
                        )
                ...
                (bool success, bytes memory returnData) = address(plugin).call(
                    abi.encodeWithSelector(
                        plugin.execute.selector,
                        caller,
                        desc,
    ...
    function _ensureBalance(address tokenOut, uint256 balanceBefore)
        private
        view
    {
        require(
            _balance(tokenOut) >= balanceBefore,
            "Sanwo: Insufficient balance after payment!"
        );
    }
    ...
    function isApproved(address pluginAddress) external view returns (bool) {
        return _isApproved(pluginAddress);
    }

    // Internal function to check if a plugin address is approved.
    function _isApproved(address pluginAddress) internal view returns (bool) {
